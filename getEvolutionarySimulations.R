getEvolutionarySimulations = function(noOfSims,mode,...){
  ...
  if (mode=="Stasis"){
    ...
    evolutionarySimulationList=list()
    for (i in 1:noOfSims){
      stasisValues=rnorm(n=length(t),mean=mean,sd=sqrt(variance))
  ...
  if (mode == "Ornstein-Uhlenbeck"){
    ...
    x0=parameters[[9]]
    for (i in 1:noOfSims){
      noiseIncrements=rnorm(n=length(t)-1,
                            mean=0,
                            sd=sqrt(diff(t)))
      ouval=rep(NA,length(t))
      ouval[1]=x0
      for (j in 2:length(t)){
        ouval[j]=ouval[j-1] + 0.001*(theta*(mu-ouval[j-1]))+sigma*noiseIncrements[j-1]
      }
